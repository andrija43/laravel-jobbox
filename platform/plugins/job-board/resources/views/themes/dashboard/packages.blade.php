                ...
                <div class="col">
                    <div class="card panel-white hover-up overflow-hidden">
                        @if ($package->percent_save)
                            <span class="position-absolute end-0 badge rounded-pill bg-danger mt-2 me-2 z-index-9 text-center" style="line-height: 8px">
                                {{ $package->percent_save_text }}
                                <span class="visually-hidden">{{ trans('plugins/job-board::dashboard.save') }}</span>
                            </span>
                        @endif

                        <div class="panel-head background-header-packages d-flex justify-content-center align-items-center" @if ($package->is_default) style="background-color: var(--primary-color) !important; color: #fff" @endif>
