@if ($jobApplication)
    ...
    <p>{{ trans('plugins/job-board::job-application.tables.position') }}:
        <a
            href="{{ $jobApplication->job->url }}"
            target="_blank"
        ><i>{{ $jobApplication->job->name }}</i> <i class="fa fa-external-link"></i></a>
    </p>
    ...
    @if (!$jobApplication->is_external_apply)
        <p>{{ trans('plugins/job-board::job-application.tables.name') }}:
            @if ($jobApplication->account->id && $jobApplication->account->is_public_profile)
                <a
                    href="{{ $jobApplication->account->url }}"
                    target="_blank"
                >{{ $jobApplication->account->name }} <i class="fa fa-external-link"></i></a>
            @else
    ...
    @if (!$jobApplication->is_external_apply)
        @if ($jobApplication->resume)
            <p>{{ trans('plugins/job-board::job-application.tables.resume') }}:
                <a
                    ...
                    target="_blank"
                >{{ trans('plugins/job-board::job-application.tables.download_resume') }} <i
        ...
        @if ($jobApplication->cover_letter)
            <p>{{ trans('plugins/job-board::job-application.tables.cover_letter') }}:
                <a
                    href="{{ RvMedia::url($jobApplication->cover_letter) }}"
                    target="_blank"
                ><i>{{ RvMedia::url($jobApplication->cover_letter) }}</i> <i class="fa fa-external-link"></i></a>
            </p>
        @endif
    @endif
