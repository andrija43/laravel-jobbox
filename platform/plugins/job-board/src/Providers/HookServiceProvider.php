...
class HookServiceProvider extends ServiceProvider
{
    public function boot(): void
    {
        ...
        add_action(BASE_ACTION_TOP_FORM_CONTENT_NOTIFICATION, function (Request $request, BaseModel|string|null $model = null) {
            ...
            echo Blade::render(<<<BLADE
                <div class="alert alert-warning">
                    {{ __('plugins/job-board::dashboard.employer_no_companies') }}
                </div>
            BLADE);
        }, 0, 2);
    }
    ...
    {
        switch ($menuId) {
            case 'cms-plugins-job-board-main':
                if (! Auth::user()->hasPermission('job-applications.index')) {
                    return $number;
                }

                $attributes = [
                    'class' => 'badge badge-success menu-item-count job-board-count',
                    'style' => 'display: none;',
                ];

                return Html::tag('span', '', $attributes)->toHtml();

            case 'cms-plugins-job-board-application':
                $attributes = [
                    'class' => 'badge badge-success menu-item-count pending-applications',
                    'style' => 'display: none;',
                ];

                return Html::tag('span', '', $attributes)->toHtml();

            case 'cms-plugins-job-board-company':
                $attributes = [
                    'class' => 'badge badge-success menu-item-count pending-companies',
                    'style' => 'display: none;',
                ];

                return Html::tag('span', '', $attributes)->toHtml();

            case 'cms-plugins-job-board-jobs':
                $attributes = [
                    'class' => 'badge badge-success menu-item-count pending-jobs',
                    'style' => 'display: none;',
                ];

                return Html::tag('span', '', $attributes)->toHtml();

            case 'cms-plugins-job-board-accounts':
                $attributes = [
                    'class' => 'badge badge-success menu-item-count pending-accounts',
                    'style' => 'display: none;',
                ];
