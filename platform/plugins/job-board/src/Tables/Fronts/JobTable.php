...
class JobTable extends TableAbstract
{
    public function setup(): void
    {
        $this
            ->model(Job::class)
            ->addActions([
                Action::make('analytics')->route('public.account.jobs.analytics')->label(__('Analytics'))->color(
                    'info'
                ),
    ...
    public function ajax(): JsonResponse
    {
        $data = $this->table
            ->eloquent($this->query())
            ->editColumn('name', function (Job $item) {
                return Html::link(route('public.account.jobs.edit', $item->getKey()), $item->name);
            })
